...
-- @searchTextPattern: CLR=System.String, DbType=String, NpgsqlDbType=Varchar, Nullable=True, Value=%needle10f%
-- @searchTextPattern0: CLR=System.String, DbType=String, NpgsqlDbType=Varchar, Nullable=True, Value=%needle10f%
-- @p19: CLR=System.Int32, DbType=Int32, NpgsqlDbType=Integer, Nullable=False, Value=20
-- @p: CLR=System.Int32, DbType=Int32, NpgsqlDbType=Integer, Nullable=False, Value=0
SELECT s0."Id", s0."AgencyId", s0."AreaSquareMeters", s0."BalconyCount", s0."Bathrooms", s0."Condition", s0."CreatedAtUtc", s0."CreatedByUserId", s0."Currency", s0."FurnishingStatus", s0."GeocodedDisplayName", s0."GeocodingProviderKey", s0."GeocodingResultReference", s0."HasBasement", s0."HeatingType", s0."IsExchangePossible", s0."Latitude", s0."ListingType", s0."LocationConfirmedAtUtc", s0."LocationPrecision", s0."Longitude", s0."ModifiedAtUtc", s0."Orientation", s0."ParkingSpaces", s0."Price", s0."PropertyType", s0."Rooms", s0."Status", s0."YearBuilt", s0."YearRenovated", l5."ListingId", l5."ApartmentType", l5."Floor", l5."HasElevator", l5."TotalFloors", l6."ListingId", l6."HouseType", l6."NumberOfFloors", l6."YardAreaSquareMeters", l7."ListingId", l7."CommercialType", l8."ListingId", l8."LandType"
FROM (
    ...
    INNER JOIN (
        ...
        ) AND (l0."Title" ILIKE @searchTextPattern ESCAPE '\' OR (l0."City" IS NOT NULL AND l0."City" ILIKE @searchTextPattern0 ESCAPE '\') OR (l0."Municipality" IS NOT NULL AND l0."Municipality" ILIKE @searchTextPattern0 ESCAPE '\') OR (l0."Neighborhood" IS NOT NULL AND l0."Neighborhood" ILIKE @searchTextPattern0 ESCAPE '\'))
